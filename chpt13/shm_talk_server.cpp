...
int main(int argc, char* argv[]) {
  ...
  /* map file to memory , which can be read and write(decided by parameter
   * prot)*/
  ...
  while (!stop_server) {
    ...
    for (int i = 0; i < number; i++) {
      ...
      if (sockfd == listenfd) {
        ...
        /* store client data in connected client array */
        users[user_count].address = client_address;
        users[user_count].connfd = connfd;

        /* build pipe between father and child process*/
        ...
        if (pid < 0) {
          ...
          /*Closing these file descriptors in child process is to finally close
           * these file descriptors in the main process*/
          close(epollfd);
          close(listenfd);
        ...
        /* handle signal event (with uni-event)*/
        int sig;
        char signals[1024];
